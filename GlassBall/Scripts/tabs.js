$(function () {
    ...
    $('#myTab a[href="#addTab"]').on('click', function () { // Click event on the "Add Tab" button
        		...
        		var tablabel = prompt("Enter the new file name", "");
        ...
        $('ul#myTab li:last-child').before('<li id="li' + (nbrLiElem + 1) + '"><a href="#tab' + (nbrLiElem + 1) + '" role="tab" data-toggle="tab">' +tablabel+ ' <button type="button" class="btn btn-warning btn-xs" onclick="removeTab(' + (nbrLiElem + 1) + ');"><span class="glyphicon glyphicon-remove"></span></button></a>');
        	
        // Add a <div></div> markup after the last-child of the <div class="tab-content">
        $('div.tab-content div:last-child').after('<div class="tab-pane" id="tab' + (nbrLiElem + 1) + '"><textarea></textarea></div>');
